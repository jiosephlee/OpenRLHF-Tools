#!/bin/bash
#
# TDC GRPO Training Script — DISTRIBUTED (non-hybrid) mode — FIXED
#
# Actor and vLLM run on SEPARATE GPUs (no colocation, no sleep modes).
# NCCL/IB config copied from working colocated script for dgx-b200 cluster.
#
# Usage:
#   export WANDB_API_KEY=...   # required for wandb tracking
#   # SLURM: sbatch scripts/train_grpo_tdc_distributed_fixed.sh <task_name> <model_path> [learning_rate]
#   # Direct: bash scripts/train_grpo_tdc_distributed_fixed.sh <task_name> <model_path> [learning_rate] [num_gpus]
#
# Examples:
#   sbatch scripts/train_grpo_tdc_distributed_fixed.sh AMES zai-org/GLM-4.7-Flash
#   bash scripts/train_grpo_tdc_distributed_fixed.sh hERG /path/to/glm-flash 1e-6 4
#

### SLURM DIRECTIVES ###
#SBATCH --job-name=D-grpo
#SBATCH --partition=dgx-b200
#SBATCH --output=logs/D-grpo_%j.out
#SBATCH --error=logs/D-grpo_%j.err
#SBATCH --nodes=1
...
run_task() {
    ...
    # Ensure worker processes use the same python
    ...
    echo "========================================"
    echo "TDC GRPO Training — DISTRIBUTED mode (FIXED)"
    echo "========================================"
    echo "Task: $TASK_NAME"
