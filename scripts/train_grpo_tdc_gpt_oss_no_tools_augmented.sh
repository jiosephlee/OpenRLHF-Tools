...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-aug-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-Augmented-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-aug-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-Augmented-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
HUB_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-aug-ep${MAX_EPOCHS}-${DATE_TAG}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (No-Tools, Augmented Data, MODE=$MODE, QUANT=$QUANT_LABEL)"
echo "========================================"
