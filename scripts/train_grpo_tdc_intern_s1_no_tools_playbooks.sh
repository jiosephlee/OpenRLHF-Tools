#!/bin/bash
#
# Playbooks no-tools GRPO training for Intern-S1 on TDC binary classification.
#
# Uses playbook-augmented prompts (with precomputed tool results baked into the text)
# instead of multi-turn tool calling. Input key is "text" (plain string), not "messages".
#
# Supports both colocated and distributed modes via MODE env var, plus all
# RL-specific feature flags from the GPT-OSS script.
#
# Usage:
#   # Colocated (default):
#   bash scripts/train_grpo_tdc_intern_s1_no_tools_augmented.sh
#
#   # With features:
#   TIS=1 TIS_TYPE=tis LIGER_GRPO_LOSS=1 LOSS_TYPE=dapo SMART_REPLAY=1 bash train_grpo_tdc_intern_s1_no_tools_augmented.sh
#
#   # Distributed:
#   MODE=distributed ACTOR_GPUS=2 VLLM_NUM_ENGINES=2 bash scripts/train_grpo_tdc_intern_s1_no_tools_augmented.sh
#
#   # LoRA:
#   USE_LORA=1 LEARNING_RATE=2e-5 bash scripts/train_grpo_tdc_intern_s1_no_tools_augmented.sh
#
...
CONDA_ENV="${CONDA_ENV:-/vast/projects/myatskar/design-documents/conda_env/openrlhf_tfv4}"
...
### DATA ###
DATA_DIR="$PROJECT_ROOT/data/tdc/playbooks_format"
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-s1-${N_TASKS}t-notools-pb-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-InternS1-no-tools-Playbooks-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-s1-${N_TASKS}t-notools-pb-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-InternS1-no-tools-Playbooks-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — Intern-S1-mini (No-Tools, Augmented Data)"
echo "========================================"
