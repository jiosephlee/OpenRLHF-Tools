#!/bin/bash
#
# KNN-3 (per-label, no pseudo label) no-tools GRPO training for GPT-OSS on TDC binary classification.
#
# Uses KNN-3 per-label augmented prompts WITHOUT pseudo labels (with precomputed
# RDKit descriptors + 3 nearest neighbors per class baked into the text) instead
# of multi-turn tool calling.
# Input key is "text" (plain string), not "messages".
#
# Data source: Intern-S1-recipe/DataPrepare/TDC_prepended/KNN_3_per_label_no_pseudo_label/
#
...
#   SMART_REPLAY=1 OVERSAMPLE_RATIO=2 TIS=1 TIS_TYPE=tis DEQUANT=unsloth EFFECTIVE_ROLLOUT_BATCH_SIZE=8 EFFECTIVE_MINI_GRADIENT_STEPS=2 REDUCE_OPTIMIZER=none TRAIN_MAX_TOKENS_PER_GPU=32768 LIGER_GRPO_LOSS=0 LOSS_TYPE=cispo bash train_grpo_tdc_gpt_oss_no_tools_knn3_no_pseudo_label.sh
#
#   # With features:
...
### DATA ###
DATA_DIR="$PROJECT_ROOT/data/tdc/knn3_per_label_no_pseudo_label_format"
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3npl-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-KNN3-NPL-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3npl-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-KNN3-NPL-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
HUB_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3npl-ep${MAX_EPOCHS}-${DATE_TAG}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (No-Tools, KNN-3 Per-Label No-Pseudo-Label Data, MODE=$MODE, QUANT=$QUANT_LABEL)"
echo "========================================"
