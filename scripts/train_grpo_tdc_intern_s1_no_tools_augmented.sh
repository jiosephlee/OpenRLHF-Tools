#!/bin/bash
#
# Augmented no-tools GRPO training for Intern-S1 on TDC binary classification.
#
...
### DATA ###
DATA_DIR="$PROJECT_ROOT/data/tdc/augmented_format"
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-s1-${N_TASKS}t-notools-aug-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-InternS1-no-tools-Augmented-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-s1-${N_TASKS}t-notools-aug-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-InternS1-no-tools-Augmented-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
