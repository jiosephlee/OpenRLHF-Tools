"""Build v14_no_neighbor TDC datasets in OpenAI chat format.

Same records and splits as :mod:`data.tdc.build_v14_datasets`, but the system
message only documents ``get_features`` (no neighbor tools), matching
``--tool_version v14_no_neighbor`` and ``tools_per_task_v14_no_neighbor.json``.

Usage:
    python data/tdc/build_v14_no_neighbor_datasets.py \\
        [--output-dir data/tdc/openai_format_v14_no_neighbor]
"""
...
DEFAULT_OUTPUT_DIR = os.path.join(_SCRIPT_DIR, "openai_format_v14_no_neighbor")
...
def main():
    parser = argparse.ArgumentParser(description="Build v14_no_neighbor TDC datasets")
