#!/bin/bash
#
# Unified SLURM batch script for GPT-OSS GRPO training (BF16).
#
...
#
# Usage:
#   LEARNING_RATE=1e-5 sbatch train_grpo_tdc_gpt_oss_slurm.sh
#
#   # Distributed:
...
### MODEL RESOLUTION (before module load — determines conda/CUDA) ###
...
run_task() {
    ...
    ### PRINT CONFIG ###
    echo "========================================"
    echo "TDC GRPO Training — GPT-OSS (MODE=$MODE, BF16, SLURM BATCH)"
    echo "========================================"
...
" "${TASK_NAMES[@]}"
    ...
    if [ -n "${SLURM_JOB_ID:-}" ]; then
        ln -sf "grpo-tdc-gptoss_${SLURM_JOB_ID}.out" "logs/grpo-tdc-gptoss_bf16_${SLURM_JOB_ID}.out" || true
        ln -sf "grpo-tdc-gptoss_${SLURM_JOB_ID}.err" "logs/grpo-tdc-gptoss_bf16_${SLURM_JOB_ID}.err" || true
    fi
