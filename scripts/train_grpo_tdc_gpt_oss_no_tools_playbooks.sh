#!/bin/bash
#
# Playbooks no-tools GRPO training for GPT-OSS on TDC binary classification.
#
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-pb-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-Playbooks-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-pb-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-Playbooks-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
HUB_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-pb-ep${MAX_EPOCHS}-${DATE_TAG}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (No-Tools, Playbooks Data, MODE=$MODE, QUANT=$QUANT_LABEL)"
echo "========================================"
