#!/bin/bash
#
# KNN-3 (global top-2k, no pseudo label) no-tools GRPO training for GPT-OSS on TDC binary classification.
#
# Uses KNN-3 global top-2k augmented prompts WITHOUT pseudo labels (with precomputed
# RDKit descriptors + 3 nearest neighbors per class baked into the text) instead
# of multi-turn tool calling. All 16 TDC binary classification tasks.
# Input key is "text" (plain string), not "messages".
#
# Data source: Intern-S1-recipe/DataPrepare/TDC_prepended/KNN_3_global_top2k_no_pseudo_label/
#
...
#   OVERSAMPLE_RATIO=2 TIS=1 TIS_TYPE=tis DEQUANT=unsloth EFFECTIVE_ROLLOUT_BATCH_SIZE=8 EFFECTIVE_MINI_GRADIENT_STEPS=2 REDUCE_OPTIMIZER=none TRAIN_MAX_TOKENS_PER_GPU=32768 LIGER_GRPO_LOSS=0 LOSS_TYPE=cispo bash train_grpo_tdc_gpt_oss_no_tools_knn3_global_top2k_no_pseudo_label.sh
#
#   # With features:
#   EFFECTIVE_ROLLOUT_BATCH_SIZE=8 EFFECTIVE_MINI_GRADIENT_STEPS=2 TIS=1 TIS_TYPE=tis REDUCE_OPTIMIZER=adam_offload TRAIN_MAX_TOKENS_PER_GPU=32768 LIGER_GRPO_LOSS=1 LOSS_TYPE=dapo SMART_REPLAY=1 DEQUANT=unsloth bash train_grpo_tdc_gpt_oss_no_tools_knn3_global_top2k_no_pseudo_label.sh
#
#   # LoRA:
...
### MULTI-TASK: 16 TDC binary classification tasks ###
TASK_NAMES=(BBB_Martins Bioavailability_Ma HIA_Hou PAMPA_NCATS CYP2C9_Substrate_CarbonMangels CYP2D6_Substrate_CarbonMangels CYP3A4_Substrate_CarbonMangels DILI SARSCoV2_3CLPro_Diamond SARSCoV2_Vitro_Touret Carcinogens_Lagunin hERG ClinTox Skin_Reaction Pgp_Broccatelli AMES)
TASK_LABEL="Base"
...
### DATA ###
DATA_DIR="$PROJECT_ROOT/data/tdc/knn3_global_top2k_no_pseudo_label_format"
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3g2k-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-KNN3-G2K-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3g2k-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-GPTOss-${QUANT_LABEL}-no-tools-KNN3-G2K-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
HUB_NAME="grpo-tdc-gptoss-${QUANT_LABEL}-${N_TASKS}t-notools-knn3g2k-ep${MAX_EPOCHS}-${DATE_TAG}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (No-Tools, KNN-3 Global-Top2k No-Pseudo-Label Data, MODE=$MODE, QUANT=$QUANT_LABEL)"
echo "========================================"
...
python -m openrlhf.cli.train_ppo_ray \
    ...
    --freeze_router \
    --aux_loss_coef 0 \
    --knn_pseudo_labels_path "$PROJECT_ROOT/data/tdc/metadata/knn3_pseudo_labels.json" \
    $QUANT_FLAGS \
    $MODE_FLAGS \
    $OPTIONAL_FLAGS \
    $EXTRA_ARGS \
