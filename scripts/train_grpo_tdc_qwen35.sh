#!/bin/bash
#
# Unified interactive script for Qwen3.5-35B-A3B GRPO training.
#
# Adaped from train_grpo_tdc_gpt_oss.sh for Qwen3.5-35B-A3B (35B MoE, 3B active).
# Uses the Qwen3 Coder XML tool-calling format:
...
#
#   # Distributed:
#   MODE=distributed ACTOR_GPUS=4 VLLM_NUM_ENGINES=4 bash scripts/train_grpo_tdc_qwen35.sh
#
...
#   ASYNC_ADVANTAGE=4                    # Scale factor for colocated
#   COLO_EVAL_STEPS=32                   # Eval frequency (global steps) for colocated
...
#   OVERSAMPLE_RATIO=1.6                 # Oversample ratio (default: 1.6)
...
#   LANGUAGE_MODEL_ONLY=1                 # Skip vision encoder (default: 1 for text-only TDC)

### MODEL PATH ###
PRETRAIN_PATH="${PRETRAIN_PATH:-Qwen/Qwen3.5-35B-A3B}"
...
### DATA ###
# Qwen3.5 uses the same OpenAI-format data (model-agnostic messages + apply_chat_template)
...
if [ "$MODE" = "colocated" ]; then
    MODE_TAG="colo"
    RUN_NAME="grpo-tdc-qwen35-${N_TASKS}t-${TOOL_VERSION}-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-Qwen35-colo-$TASK_LABEL"
else
    MODE_TAG="dist-${LAYOUT_TAG}"
    RUN_NAME="grpo-tdc-qwen35-${N_TASKS}t-${TOOL_VERSION}-ep${MAX_EPOCHS}${SUFFIX}-${MODE_TAG}-${DATE_TAG}"
    WANDB_GROUP="TDC-Qwen35-dist-${LAYOUT_TAG}-$TASK_LABEL"
fi
RUN_ID="${RUN_NAME}"
HUB_NAME="grpo-tdc-qwen35-${N_TASKS}t-${TOOL_VERSION}-ep${MAX_EPOCHS}-${DATE_TAG}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — Qwen3.5-35B-A3B (MODE=$MODE)"
echo "========================================"
...
### TRAINING ###
RUN_LOG="$RUNS_DIR/run_qwen35.log"
