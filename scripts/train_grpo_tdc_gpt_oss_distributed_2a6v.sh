#!/bin/bash
#
# GPT-OSS GRPO training — DISTRIBUTED (2 actor + 6 vLLM GPUs).
#
...
#   3. Run:                      bash scripts/train_grpo_tdc_gpt_oss_distributed_2a6v.sh [model_path] [learning_rate]
#
# Example:
#   bash scripts/train_grpo_tdc_gpt_oss_distributed_2a6v.sh openai/gpt-oss-20b 1e-6
#
...
### RUN CONFIG ###
RUN_ID="GPTOss-grpo-dist-2a6v-${TASK_LABEL}_$(date +%Y-%m-%d_%H-%M-%S)_lr${LEARNING_RATE}"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (DISTRIBUTED, 2 actor + 6 vLLM GPUs)"
echo "========================================"
...
echo "W&B: project=$WANDB_PROJECT group=TDC-GPTOss-dist-2a6v-$TASK_LABEL run=$RUN_ID"
echo "========================================"
...
python -m openrlhf.cli.train_ppo_ray \
    ...
    --wandb_group "TDC-GPTOss-dist-2a6v-$TASK_LABEL" \
