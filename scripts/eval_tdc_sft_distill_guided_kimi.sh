...
# with a Kimi teacher.
#
# This is a standalone Kimi analogue of eval_tdc_sft_distill_guided_glm.sh.
#
# Usage:
#   bash scripts/eval_tdc_sft_distill_guided_kimi.sh
#   EVAL_SPLIT=val bash scripts/eval_tdc_sft_distill_guided_kimi.sh
#   PRETRAIN_PATH=/path/to/kimi bash scripts/eval_tdc_sft_distill_guided_kimi.sh
#
set -eo pipefail

PRETRAIN_PATH="${PRETRAIN_PATH:-moonshotai/Kimi-K2.5}"
...
CHAT_PROTOCOL="kimi_k2"
RUN_NAME="eval-tdc-kimi-${MODEL_TAG}-${N_TASKS}t-${TOOL_VERSION}-${DATA_TAG}-${EVAL_SPLIT}-${MODE_TAG}-${DATE_TAG}"
...
WANDB_GROUP="${WANDB_GROUP:-TDC-Kimi-eval-${MODEL_TAG}-${DATA_TAG}-${EVAL_SPLIT}-${MODE_TAG}}"
...
echo "========================================"
echo "TDC Eval Only — Kimi (MODE=$MODE, MODEL=$MODEL_TAG)"
echo "========================================"
...
TRAIN_CMD=(
    ...
    --vllm_stop_strings "<|tool_calls_section_end|>" "<|tool_call_section_end|>"
