...
- **Full Per-Sample Trace Dumps**: When `--save_all_traces` is enabled (default `True`) and `OPENRLHF_FULL_TRACE_DIR` is set, every rollout *and* every eval sample is appended as JSONL to `$OPENRLHF_FULL_TRACE_DIR/{train|eval}/<week>/<run_name>/full_traces/{rollout|eval}_step_<N>.jsonl` (default location: `/vast/projects/myatskar/design-documents/joseph/therapeutic-tuning`). Writes are **off-thread**: producer enqueues to a bounded `queue.Queue` and a daemon consumer thread (`openrlhf.utils.trace_writer.TraceWriter`) appends to disk. Under back-pressure (slow shared-FS, queue full, or per-step wall-clock budget exhausted) records are dropped silently and counted; the trainer never blocks on trace I/O. Knobs: `OPENRLHF_TRACE_QUEUE_SIZE` (default 4096), `OPENRLHF_TRACE_BUDGET_SEC` (default 2.0). Drop counters surface in wandb under `trace/{rollout,eval}_{dropped_full,dropped_budget,budget_exceeded_steps,bytes_written}`. The schemas are now intentionally asymmetric: rollout records exclude `trace_messages` and the always-null eval-only fields; eval records include `trace_messages` only when `--save_distill_trace_messages` is set (default off; flip on for SFT distillation runs). The per-step `traces/step{N}_groups.json` artifact has been retired; the curated `traces/group_trace_step{N}_{mixed,needle}.json` showcase files (now also off-path with a 5s timeout) cover the same use case. Optional `OPENRLHF_FULL_TRACE_KEEP_LAST=N` enables FIFO retention. `--save_sft_distill_traces` remains a deprecated alias for `--save_all_traces`.
