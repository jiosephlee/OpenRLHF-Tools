#!/bin/bash
#
# Prepended Tools GRPO training for GPT-OSS on TDC binary classification.
#
# Uses enriched prompts with precomputed therapeutic tool outputs
# (molecule profile, functional groups, ring systems, ADME, structural alerts,
# salt removal, metabolite prediction, and 3D properties for relevant tasks)
# baked into the text. No multi-turn tool calling at runtime.
# Input key is "text" (plain string), not "messages".
#
# Data source: data/tdc/prepended_tools_format/
# Build first: python scripts/build_knn3_tools_dataset.py
#
...
# Usage:
#   # MXFP4 (default):
#   bash scripts/train_grpo_tdc_gpt_oss_prepended_tools.sh
#
#   # Unsloth BF16:
...
#
#   # LoRA:
#   USE_LORA=1 LEARNING_RATE=2e-5 DEQUANT=unsloth bash scripts/train_grpo_tdc_gpt_oss_prepended_tools.sh
#
#   # Distributed:
#   MODE=distributed ACTOR_GPUS=1 VLLM_NUM_ENGINES=1 DEQUANT=unsloth bash scripts/train_grpo_tdc_gpt_oss_prepended_tools.sh
#
...
for t in "${TASK_NAMES[@]}"; do
    ...
    if [ ! -f "$f" ]; then
        ...
        echo "Have you run: python scripts/build_knn3_tools_dataset.py ?"
...
### PRINT CONFIG ###
echo "========================================"
echo "TDC GRPO Training — GPT-OSS (Prepended Tools, MODE=$MODE, QUANT=$QUANT_LABEL)"
echo "========================================"
